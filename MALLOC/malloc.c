...
//内存池(64字节对齐)
__align(64) u8 mem1base[MEM1_MAX_SIZE] __attribute__((at(0x24050000)));                 //内部SRAM内存池
//外部SDRAM内存池,从第32M开始，到64M结束，前面32M用于缓存和地图数据处理
//0xC0336680 对齐-> 0xC0336700 跳过-> 0xC2000000
__align(64) u8 mem2base[MEM2_MAX_SIZE] __attribute__((at(0xC2000000)));     //外部SDRAM内存池,
...
//内存管理表
u32 mem1mapbase[MEM1_ALLOC_TABLE_SIZE] __attribute__((at(0x24050000 + MEM1_MAX_SIZE)));   //内部SRAM内存池MAP
u32 mem2mapbase[MEM2_ALLOC_TABLE_SIZE] __attribute__((at(0xC2000000 + MEM2_MAX_SIZE)));	//外部SDRAM内存池MAP
...
//内存管理参数	   
const u32 memtblsize[SRAMBANK_] = { MEM1_ALLOC_TABLE_SIZE,MEM2_ALLOC_TABLE_SIZE,MEM3_ALLOC_TABLE_SIZE,
                                MEM4_ALLOC_TABLE_SIZE,MEM5_ALLOC_TABLE_SIZE,MEM6_ALLOC_TABLE_SIZE };	//内存表大小
const u32 memblksize[SRAMBANK_] = { MEM1_BLOCK_SIZE,MEM2_BLOCK_SIZE,MEM3_BLOCK_SIZE,
                                MEM4_BLOCK_SIZE,MEM5_BLOCK_SIZE,MEM6_BLOCK_SIZE };					//内存分块大小
const u32 memsize[SRAMBANK_] = { MEM1_MAX_SIZE,MEM2_MAX_SIZE,MEM3_MAX_SIZE,

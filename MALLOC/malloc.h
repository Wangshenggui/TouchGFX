...
#define SRAMBANK_ 	6		//定义支持的SRAM块数.	


//mem1内存参数设定.mem1是H7内部的AXI内存.
#define MEM1_BLOCK_SIZE			64  	  						//内存块大小为64字节
#define MEM1_MAX_SIZE			0x2D000 //最大管理内存 180KB（TouchGFX占用320KB）,H7的AXI内存总共512KB
#define MEM1_ALLOC_TABLE_SIZE	MEM1_MAX_SIZE/MEM1_BLOCK_SIZE 	//内存表大小

//mem2内存参数设定.mem2是外部的SDRAM内存
#define MEM2_BLOCK_SIZE			64  	  						//内存块大小为64字节
#define MEM2_MAX_SIZE			30 * 1024 *1024  			    //最大管理内存30720K
...
struct _m_mallco_dev
{
	...
	u8* membase[SRAMBANK_];				//内存池 管理SRAMBANK个区域的内存
	u32* memmap[SRAMBANK_]; 				//内存管理状态表
	u8  memrdy[SRAMBANK_]; 				//内存管理是否就绪
};

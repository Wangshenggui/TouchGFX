...
SD_Error SD_Init(void)
{
	...
	GPIO_Set(GPIOC, 0X1F << 8, GPIO_MODE_AF, GPIO_OTYPE_PP, GPIO_SPEED_HIGH, GPIO_PUPD_PU);	//PC8,9,10,11,12复用功能输出	
	GPIO_Set(GPIOD, 1 << 2, GPIO_MODE_AF, GPIO_OTYPE_PP, GPIO_SPEED_HIGH, GPIO_PUPD_PU);		//PD2复用功能输出	
	...
	if ((errorstatus == SD_OK) || (MULTIMEDIA_CARD == CardType))
	{
		...
		{
			clkdiv = SDMMC_TRANSFER_CLK_DIV + 4;	//V1.1/V2.0卡，设置最高200/[2*(2+4)]=16.67Mhz
		}
		else clkdiv = SDMMC_TRANSFER_CLK_DIV;	//SDHC等其他卡，设置最高200/(2*2)=50Mhz

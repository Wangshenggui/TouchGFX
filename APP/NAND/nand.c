...
u8 NAND_Init(void)
{
	...
	((FMC_Bank3_TypeDef*)FMC_Bank3_R_BASE)->PCR = tempreg;		//设置NAND控制寄存器
	...
	((FMC_Bank3_TypeDef*)FMC_Bank3_R_BASE)->PMEM = tempreg;	//设置通用时序寄存器 
	((FMC_Bank3_TypeDef*)FMC_Bank3_R_BASE)->PATT = 0;			//设置特性时序寄存器 

	((FMC_Bank3_TypeDef*)FMC_Bank3_R_BASE)->PCR |= 1 << 2;		//使能存储区3 
	((FMC_Bank1_TypeDef*)FMC_Bank1_R_BASE)->BTCR[0] |= (u32)1 << 31;	//使能FMC  
	NAND_Reset();       		//复位NAND
	//    printf("yy2");
	delay_ms(100);
	//    printf("yy3");
	...
	*(vu8*)(NAND_ADDRESS | NAND_CMD) = NAND_AREA_TRUE1;
	...
	else
	{
		...
		for (res = 0; res < eccnum; res++)
		{
			...
			((FMC_Bank3_TypeDef*)FMC_Bank3_R_BASE)->PCR |= 1 << 6;							//使能ECC校验 
			for (i = 0; i < NAND_ECC_SECTOR_SIZE; i++)				//读取NAND_ECC_SECTOR_SIZE个数据
			{
				*(vu8*)pBuffer++ = *(vu8*)NAND_ADDRESS;
			}
			while (!(((FMC_Bank3_TypeDef*)FMC_Bank3_R_BASE)->SR & (1 << 6)));					//等待FIFO空	
			SCB_CleanInvalidateDCache();					//清除无效的D-Cache   
			nand_dev.ecc_hdbuf[res + eccstart] = ((FMC_Bank3_TypeDef*)FMC_Bank3_R_BASE)->ECCR;//读取硬件计算后的ECC值
			((FMC_Bank3_TypeDef*)FMC_Bank3_R_BASE)->PCR &= ~(1 << 6);						//禁止ECC校验
		}

...
{
	u8 sta,len;  
	if(UART_USMART_Struct.Rx_State==1)//串口接收完成？
	{
		len=UART_USMART_Struct.Rx_len;	//得到此次接收到的数据长度
        UART_USMART_Struct.Rx_len=0;
		UART_USMART_Struct.Buff[len]='\0';	//在末尾加入结束符. 
		sta=usmart_dev.cmd_rec(UART_USMART_Struct.Buff);//得到函数各个信息
		if(sta==0)usmart_dev.exe();	//执行函数 
		else 
		{  
			len=usmart_sys_cmd_exe(UART_USMART_Struct.Buff);
		...
		UART_USMART_Struct.Rx_State=0;//状态寄存器清空	    
	}
}

...
////////////////////////////自己定义的一个标记USB状态的寄存器///////////////////
USB_GlobalVariableStructure USB_GlobalVariableStruct={0};
////////////////////////////////////////////////////////////////////////////////
...
{     
	switch(lun)
	{
		case 0://SPI FLASH
			*block_size=512;  
			*block_num=1024*1024*32/512;	//SPI FLASH的前面25M字节,文件系统用
			break;
		case 1://NAND FLASH
//			*block_size=512;  
...
int8_t  STORAGE_IsReady (uint8_t lun)
{ 
	USB_GlobalVariableStruct.USB_STATUS_REG|=0X10;//标记轮询
	return 0;
} 
...
{
	int8_t res=0;
	USB_GlobalVariableStruct.USB_STATUS_REG|=0X02;//标记正在读数据 
	switch(lun)
	{
		case 0://SPI FLASH
...
//			res=SD_ReadDisk(buf,blk_addr,blk_len);
	...
	if(res)
	{
		USB_GlobalVariableStruct.USB_STATUS_REG|=0X08;//读错误! 
	}  
	return res;
}
...
{
	int8_t res=0;
	USB_GlobalVariableStruct.USB_STATUS_REG|=0X01;//标记正在写数据 
	switch(lun)
	{
		case 0://SPI FLASH
...
//            res=SD_WriteDisk(buf,blk_addr,blk_len);
	...
	if(res)
	{
		USB_GlobalVariableStruct.USB_STATUS_REG|=0X04;//写错误!	 
	} 
	return res; 
}
